                ...
                iso|squashfs|tgz|tar|torrent)
                    ...
                    then
                        case "$url" in
                            ...
                            *.torrent)
                                echo "Torrent specified, obtaining from ${url}..."
                                echo "Saving torrent file to ${dest}/$(basename ${url})"
                                wget "${url}" -O "${dest}/$(basename ${url}).torrent"
                                echo "Saving squashfs to ${dest}/filesystem.squashfs"
                                ctorrent -E 0 -e 0 -s "${dest}/filesystem.squashfs" "${dest}/$(basename ${url}).torrent"
                            ;;

                            *)
                                log_begin_msg "Trying wget ${url} -O ${dest}/$(basename ${url})"
                                                                # important to note that we need to use the copied in wget, busybox version is missing options
                                /bin/wget --tries 10 --timeout=20 "${url}" -O "${dest}/$(basename ${url})"
                                ;;
                        esac
                    else
